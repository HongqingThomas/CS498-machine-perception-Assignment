...
class trackingEvaluation(object):
    ...
    def __init__(self, t_sha, gt_path="./scripts/KITTI", max_truncation = 0, min_height = 25, max_occlusion = 2, \
        mail=None, cls="car", eval_3diou=True, eval_2diou=False, thres=None):
        # get number of sequences and
        # get number of frames per sequence from test mapping
        # (created while extracting the benchmark)
        all_sequences = os.listdir(t_sha)
        # print(all_sequences)
        
        self.n_frames         = []
        self.sequence_name    = []
        for i, seq_name in enumerate(all_sequences):
            
            self.sequence_name.append(seq_name[:-4])
            
            # super crude for getting num frames but ok.
            oxt_root = 'data/oxts/training'
            oxts_path = os.path.join(oxt_root, seq_name)
            oxts_imu = load_oxts(oxts_path)            
            self.n_frames.append(oxts_imu.shape[0])
    ...
    # def compute3rdPartyMetrics(self, threshold=3):
        ...
        for seq_idx in range(len(self.groundtruth)):
            ...
            for f in range(len(seq_gt)):        # go through each frame
                ...
                association_matrix = np.stack((row_ind, col_ind), axis=1)
...
def evaluate(result_sha,mail,eval_3diou,eval_2diou,thres):
    """
    ...
    for c in ("cyclist", "pedestrian", "car"):
        e = trackingEvaluation(t_sha=result_sha, mail=mail,cls=c,eval_3diou=eval_3diou,eval_2diou=eval_2diou,thres=thres)
...
if __name__ == "__main__":
    ...
    # evaluate results
    success = evaluate(result_sha,mail,eval_3diou,eval_2diou,thres)