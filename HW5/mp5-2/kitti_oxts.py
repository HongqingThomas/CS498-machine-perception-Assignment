...
# numba is a library for optimizing python code
...
# Need to update both the trks and trackers
def ego_motion_compensation(frame, trks, trackers, oxts, calib):
    # inverse ego motion compensation, move trks from the last frame of coordinate to the current frame for matching
    assert len(trackers) == len(trks), 'error'
    ...
    for index in range(len(trackers)):
        ...
        # print(trackers[index].x[:3])
        try:
            trackers[index].x[:3] = copy.copy(compensated).reshape((-1))
        except:
            trackers[index].x[:3] = copy.copy(compensated).reshape((-1, 1))
        # print(trackers[index].x[:3])
